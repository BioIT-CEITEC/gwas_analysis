...
command = "plink --bfile " + str(snakemake.output.founders_filt).replace(".bed","") + " --extract " + snakemake.output.prunesnp + " --genome --out " + str(snakemake.output.founders_txt).replace(".genome","") + " >> " + log_filename + " 2>&1 " 
f = open(log_filename, 'at')
f.write("## COMMAND: "+command+"\n")
f.close()
shell(command)
